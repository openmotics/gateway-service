...
class ApiSystemConfigTests(unittest.TestCase):
    ...
    def test_get_rfid(self):
        with mock.patch.object(self.rfid_controller, 'load_rfids', return_value=self.all_test_rfids):
            auth_token = AuthenticationToken(self.test_admin_1, token='test-token', expire_timestamp=int(time.time() + 3600), login_method=LoginMethod.PASSWORD)
    ...
    def test_get_rfid_normal_user(self):
        with mock.patch.object(self.rfid_controller, 'load_rfids', return_value=self.all_test_rfids):
            auth_token = AuthenticationToken(self.test_user_1, token='test-token', expire_timestamp=int(time.time() + 3600), login_method=LoginMethod.PASSWORD)
            resp = self.web.get_rfids(auth_token=auth_token)
            rfids = json.loads(resp)
