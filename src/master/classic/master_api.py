...
def event_triggered(master_version):
    """
    The message sent by the Master to inform the Gateway of certain events. The `event_type` field holds
    information about how to interprete the payload bytes. If no `event_type` field is present, 0 can be assumed
    * Event type 0: Classic event generated by BA 60 (bytes[0] contains the code)
